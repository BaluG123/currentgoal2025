...
const EssayDetailScreen = ({ route, navigation }) => {
    ...
    <View style={[styles.container, { backgroundColor: theme.background }]}>
      ...
      <LinearGradient colors={GRADIENTS.primary} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerInfo}>
            <Text style={styles.headerTitle}>{essay.title}</Text>
            <Text style={styles.headerSubtitle}>Kannada Essay</Text>
          </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        <View style={[styles.card, { backgroundColor: theme.card, borderColor: theme.border }]}>
          <Text style={[styles.title, { color: theme.textPrimary }]}>{essay.title}</Text>
          <Text style={[styles.body, { color: theme.textSecondary }]}>{essay.description}</Text>
        </View>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  card: { borderRadius: 12, borderWidth: 1, borderColor: '#E5E7EB', padding: 16, marginTop: 16 },
...
export default EssayDetailScreen;
