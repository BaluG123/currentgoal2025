...
const EssayListScreen = ({ navigation }) => {
      ...
      <LinearGradient colors={GRADIENTS.primary} style={styles.header} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }}>
        <View style={styles.headerContent}>
          ...
          <View style={styles.headerInfo}>
            <Text style={styles.headerTitle}>Kannada Essays</Text>
            <Text style={styles.headerSubtitle}>Topics and details</Text>
          </View>
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        {ESSAYS.map((essay) => (
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('EssayDetail', { essay })}
            activeOpacity={0.8}
          >
            <View style={styles.itemHeader}>
              <Text style={[styles.itemTitle, { color: theme.textPrimary }]}>{essay.title}</Text>
